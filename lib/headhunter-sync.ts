...
const TOKEN_ENDPOINT = `${HEADHUNTER_API_BASE.replace(/\/+$/, '')}/token`
...
async function ensureHeadHunterAccessToken(): Promise<string | null> {
  const configured = process.env.HH_API_TOKEN?.trim()
...
async function getHeadHunterHeaders() {
  const headers = new Headers({
    Accept: 'application/json',
    'User-Agent': getHeadHunterUserAgent(),
  })

  const token = await ensureHeadHunterAccessToken()
  if (token) {
...
function parseTelegramChannels(value?: string | null) {
  if (!value) return []
  return value
    .split(',')
    .map((v) => v.trim())
...
function detectRemoteFromText(text: string) {
  const haystack = text.toLowerCase()
  return (
    haystack.includes('remote') ||
    haystack.includes('удален') ||
    haystack.includes('удалён') ||
    haystack.includes('remotely') ||
    haystack.includes('fully remote')
  )
}
...
function guessCityFromText(text: string) {
  const haystack = text.toLowerCase()
  const cities = [
    { ru: 'астана', en: 'astana', value: 'Астана' },
    { ru: 'алматы', en: 'almaty', value: 'Алматы' },
    { ru: 'шымкент', en: 'shymkent', value: 'Шымкент' },
    { ru: 'караганда', en: 'karaganda', value: 'Караганда' },
    { ru: 'актау', en: 'aktau', value: 'Актау' },
    { ru: 'атырау', en: 'atyrau', value: 'Атырау' },
    { ru: 'актобе', en: 'aktobe', value: 'Актобе' },
    { ru: 'павлодар', en: 'pavlodar', value: 'Павлодар' },
    { ru: 'усть-каменогорск', en: 'ust-kamenogorsk', value: 'Усть-Каменогорск' },
  ]
  for (const city of cities) {
    if (haystack.includes(city.ru) || haystack.includes(city.en)) return city.value
  }
  return ''
}

function guessCompanyFromText(text: string) {
  const match =
    text.match(/(?:company|компания)\s*[:\-]\s*([^\n\r]{2,80})/i) ||
    text.match(/(?:работодатель)\s*[:\-]\s*([^\n\r]{2,80})/i)
  return match?.[1]?.trim() || 'Telegram Employer'
}
...
function guessTitleFromText(text: string) {
  ...
  const match = text.match(
    /\b(frontend|back[- ]?end|fullstack|devops|sre|qa|data|ml|mobile|ios|android|flutter|node\.?js|python|golang|java)\b[^\n\r]{0,60}/i
  )
  return match?.[0]?.trim() || 'IT Vacancy'
}

function extractStackFromText(text: string) {
  const haystack = text.toLowerCase()
  return STACK_KEYWORDS.filter((keyword) => haystack.includes(keyword.toLowerCase()))
}

async function fetchTelegramChannelPage(channel: string, before?: string | null) {
  const url = before ? `https://t.me/s/${channel}?before=${encodeURIComponent(before)}` : `https://t.me/s/${channel}`
  const response = await fetch(url, { cache: 'no-store' })
  if (!response.ok) {
    const errorText = await response.text().catch(() => '')
    throw new Error(`Telegram fetch failed (${response.status}) for ${channel}: ${errorText || response.statusText}`)
  }
  return response.text()
}

function parseTelegramPosts(channel: string, html: string): TelegramVacancyPost[] {
  const posts: TelegramVacancyPost[] = []
  ...
  while ((blockMatch = blockRegex.exec(html))) {
    ...
    const postRef = idMatch?.[1] || ''
    if (!postRef.startsWith(`${channel}/`)) continue
    const messageId = postRef.split('/')[1]
    if (!messageId) continue

    const textMatch = block.match(/<div class="tgme_widget_message_text[^"]*"[^>]*>([\s\S]*?)<\/div>/i)
    const rawTextHtml = textMatch?.[1]
    if (!rawTextHtml) continue

    const text = stripHtml(rawTextHtml)
    if (text.length < 30) continue

    const dateMatch = block.match(/datetime="([^"]+)"/i)
    const publishedAt = dateMatch?.[1] ? new Date(dateMatch[1]) : new Date()

    const title = guessTitleFromText(text)
    const companyName = guessCompanyFromText(text)
    const location = guessCityFromText(text)
    const remote = detectRemoteFromText(text)
    const stack = extractStackFromText(text)

    posts.push({
      externalId: `tg:${channel}:${messageId}`,
...
async function importTelegramVacancy(post: TelegramVacancyPost, placeholderPasswordHash: string) {
  const employerExternalId = `tg:${post.companyName.toLowerCase().replace(/[^a-z0-9а-яё]+/gi, '-').slice(0, 48)}`
  const employerResult = await findOrCreateHeadHunterEmployer(
    { id: employerExternalId, name: post.companyName, alternate_url: '' },
    post.location || (post.remote ? 'Remote' : ''),
    post.externalId,
  ...
  const vacancyPayload = {
    ...
    skillsRequired: post.stack.join(', '),
    salaryMin: null,
    salaryMax: null,
    salaryCurrency: DEFAULT_SALARY_CURRENCY,
    experience: 'Not specified',
    employmentType: 'Full-time',
    workFormat: post.remote ? 'Remote' : 'Onsite',
    workMode: post.remote ? 'REMOTE' : 'ONSITE',
    country: '',
    city: post.location,
    address: post.location || (post.remote ? 'Remote' : ''),
    sourceUrl: post.sourceUrl,
    externalPublishedAt: post.publishedAt,
    importedAt: new Date(),
    createdAt: post.publishedAt,
    requirements: post.stack.length > 0 ? post.stack : undefined,
  }

  const savedVacancy = await Vacancy.findOneAndUpdate(
    { source: 'HEADHUNTER', externalId: post.externalId },
    { $set: vacancyPayload },
  ...
  if (totalContentBytes > HEADHUNTER_IMPORT_LIMIT_BYTES) {
    ...
    return {
      ...
      stopReason: `Storage limit reached at ${toMegabytes(rolledBackBytes)} MB. The last incomplete vacancy was rolled back.`,
    } satisfies ImportFetchedHeadHunterVacancyResult
  }
...
async function fetchHeadHunterJson<T>(path: string, searchParams?: URLSearchParams) {
  ...
  const headers = await getHeadHunterHeaders()
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      const backoffMs = Number.isFinite(retryAfterSec)
        ...
        : Math.min(8000, 250 * 2 ** (attempt - 1))

      await new Promise((resolve) => setTimeout(resolve, backoffMs))
    } catch (error) {
      lastError = error instanceof Error ? error : new Error('Unknown HeadHunter request error')
      if (attempt === maxAttempts) {
        throw lastError
      }
      await new Promise((resolve) => setTimeout(resolve, Math.min(8000, 250 * 2 ** (attempt - 1))))
    }
  }

  throw lastError || new Error('Failed to request HeadHunter API')
}
...
export async function probeHeadHunterServerAccess() {
  ...
  await fetchHeadHunterJson<HeadHunterSearchResponse>('/vacancies', searchParams)
  return true
}

async function syncTelegramVacancies(options: SyncHeadHunterVacanciesOptions = {}): Promise<SyncHeadHunterVacanciesResult> {
  await dbConnect()
  ...
  const channels = parseTelegramChannels(process.env.TG_IMPORT_CHANNELS)
  const maxPages = toFinitePositiveNumber(process.env.TG_IMPORT_MAX_PAGES ? Number(process.env.TG_IMPORT_MAX_PAGES) : null) || 6

  await emitLog(
    'INFO',
    `Starting Telegram import for Kazakhstan IT vacancies. Channels: ${channels.length > 0 ? channels.join(', ') : '(none)'}`
  )

  if (channels.length === 0) {
    await emitLog(
      'ERROR',
      'No Telegram channels configured. Set TG_IMPORT_CHANNELS env var (comma-separated channel usernames).'
    )
    await emitProgress()
    return {
  ...
  // Keep the same cleanup/size logic.
  const cutoffDate = new Date(Date.now() - HEADHUNTER_LOOKBACK_DAYS * 24 * 60 * 60 * 1000)
  ...
  for (const channel of channels) {
    ...
    progress.currentQuery = `tg:${channel}`
    progress.currentPage = 0
    await emitLog('INFO', `Fetching Telegram channel "${channel}".`)
    await emitProgress()
    ...
    for (let page = 0; page < maxPages; page += 1) {
      ...
      const html = await fetchTelegramChannelPage(channel, before)
      const posts = parseTelegramPosts(channel, html)
      await emitLog('INFO', `Channel "${channel}": page ${page + 1}/${maxPages}, ${posts.length} posts parsed.`)
      ...
      for (const post of posts) {
        ...
        try {
          const result = await importTelegramVacancy(post, placeholderPasswordHash)
          ...
          const message = error instanceof Error ? error.message : 'Unknown Telegram vacancy processing error'
          await emitLog('ERROR', `Telegram vacancy ${post.externalId} failed: ${message}`)
          await emitProgress()
        }
      }

      // pagination: use the smallest messageId we saw as "before"
      const ids = posts
        .map((p) => p.externalId.split(':').pop() || '')
        .map((v) => Number(v))
        .filter((v) => Number.isFinite(v))
      const minId = ids.length > 0 ? Math.min(...ids) : null
      before = minId ? String(minId) : null
      if (!before || posts.length === 0) break
    }
  }
  ...
  await emitLog(
    ...
    stoppedByLimit ? 'Import stopped because the 350 MB limit was reached.' : 'Telegram import completed successfully.'
  )
  await emitProgress()
  ...
  return {
    ...
    searchTerms: channels.map((c) => `tg:${c}`),
    stoppedByLimit,
  }
}

type IrCenterVacancyRow = {
  externalId: string
  sourceUrl: string
  title: string
  companyName: string
...
function parseImportUrls(value?: string | null) {
  if (!value) return []
  return value
    .split(',')
    .map((v) => v.trim())
...
function parseSalaryRange(value: string) {
  ...
  const parts = cleaned.split('-').map((v) => Number(v.trim())).filter((v) => Number.isFinite(v))
  if (parts.length === 0) return { min: null, max: null }
  if (parts.length === 1) return { min: parts[0], max: parts[0] }
  return { min: Math.min(parts[0], parts[1]), max: Math.max(parts[0], parts[1]) }
}

async function fetchIrCenterPage(url: string) {
  const response = await fetch(url, { cache: 'no-store' })
  if (!response.ok) {
    const errorText = await response.text().catch(() => '')
    throw new Error(`ir-center fetch failed (${response.status}): ${errorText || response.statusText}`)
  }
  return response.text()
}

function parseIrCenterRows(html: string, fallbackCity: string): IrCenterVacancyRow[] {
  // The page contains many links like https://www.enbek.kz/ru/vacancy/5620742 and surrounding table cells.
  // We use a tolerant regex per row and fall back to minimal data when some columns are missing.
  const rows: IrCenterVacancyRow[] = []
  const rowRegex =
    /<tr[^>]*>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<\/tr>/gi

  let match: RegExpExecArray | null = null
  while ((match = rowRegex.exec(html))) {
    const titleCell = stripHtml(match[1] || '')
    const salaryCell = stripHtml(match[2] || '')
    const cityCell = stripHtml(match[3] || '')
    const companyCell = stripHtml(match[4] || '')
    const dateCell = stripHtml(match[5] || '')
    const scheduleCell = stripHtml(match[6] || '')

    const enbekUrlMatch = (match[1] || '').match(/https?:\/\/www\.enbek\.kz\/ru\/vacancy\/(\d+)/i)
    const enbekId = enbekUrlMatch?.[1]
    const sourceUrl = enbekId ? `https://www.enbek.kz/ru/vacancy/${enbekId}` : ''
    const externalId = enbekId ? `enbek:${enbekId}` : `ir:${Buffer.from(titleCell + companyCell).toString('base64').slice(0, 24)}`

    const publishedAt = (() => {
      const dateMatch = dateCell.match(/\b(\d{4}-\d{2}-\d{2})\b/)
      if (dateMatch?.[1]) {
    ...
    const { min, max } = parseSalaryRange(salaryCell)
    const city = cityCell || fallbackCity || guessCityFromText(`${titleCell}\n${companyCell}`)
    const description = [titleCell, companyCell ? `Компания: ${companyCell}` : '', scheduleCell ? `График: ${scheduleCell}` : '', sourceUrl ? `Источник: ${sourceUrl}` : '']
      .filter(Boolean)
      .join('\n')
      .trim()

    const remote = detectRemoteFromText(`${titleCell}\n${scheduleCell}`)
    const stack = extractStackFromText(description)

    if (!titleCell || titleCell.length < 3) continue

    rows.push({
      externalId,
      sourceUrl,
      title: titleCell,
      companyName: companyCell || 'Employer',
      city,
      salaryMin: min,
      salaryMax: max,
      publishedAt,
...
async function importIrCenterVacancy(row: IrCenterVacancyRow, placeholderPasswordHash: string) {
  const employerExternalId = `enbek-employer:${row.companyName.toLowerCase().replace(/[^a-z0-9а-яё]+/gi, '-').slice(0, 48)}`
  const employerResult = await findOrCreateHeadHunterEmployer(
    { id: employerExternalId, name: row.companyName, alternate_url: '' },
    row.city || (row.remote ? 'Remote' : ''),
    row.externalId,
    placeholderPasswordHash
  )
  ...
  const vacancyPayload = {
    ...
    skillsRequired: row.stack.join(', '),
    salaryMin: row.salaryMin,
    salaryMax: row.salaryMax,
    salaryCurrency: DEFAULT_SALARY_CURRENCY,
    experience: 'Not specified',
    employmentType: row.employmentType,
    workFormat: row.remote ? 'Remote' : 'Onsite',
    workMode: row.remote ? 'REMOTE' : 'ONSITE',
    country: '',
    city: row.city,
    address: row.city || (row.remote ? 'Remote' : ''),
    sourceUrl: row.sourceUrl,
    externalPublishedAt: row.publishedAt,
    importedAt: new Date(),
    createdAt: row.publishedAt,
    requirements: row.stack.length > 0 ? row.stack : undefined,
  }

  const savedVacancy = await Vacancy.findOneAndUpdate(
    { source: 'HEADHUNTER', externalId: row.externalId },
    { $set: vacancyPayload },
    { upsert: true, new: true, setDefaultsOnInsert: true }
  )
...
async function syncIrCenterVacancies(options: SyncHeadHunterVacanciesOptions = {}): Promise<SyncHeadHunterVacanciesResult> {
  await dbConnect()
  ...
  const urls = parseImportUrls(process.env.IR_IMPORT_URLS)
  await emitLog('INFO', `Starting ir-center import. Sources: ${urls.length > 0 ? urls.join(', ') : '(none)'}`)

  if (urls.length === 0) {
    await emitLog('ERROR', 'No IR sources configured. Set IR_IMPORT_URLS (comma-separated ir-center URLs).')
    await emitProgress()
    return {
      ...withProgressPercent(progress),
      fetched: 0,
  ...
  for (let idx = 0; idx < urls.length; idx += 1) {
    if (stoppedByLimit) break
    const url = urls[idx]
    progress.currentQuery = 'ir-center'
    progress.currentPage = idx + 1
    await emitLog('INFO', `Fetching ir-center page: ${url}`)
    await emitProgress()

    try {
      const html = await fetchIrCenterPage(url)
      const fallbackCity = guessCityFromText(stripHtml(html))
      const rows = parseIrCenterRows(html, fallbackCity)
      await emitLog('INFO', `Parsed ${rows.length} vacancies from ir-center page.`)
      ...
      for (const row of rows) {
        ...
        try {
          const result = await importIrCenterVacancy(row, placeholderPasswordHash)
          progress.totalContentBytes = result.totalContentBytes
          ...
          progress.employersCreatedCount += result.employersCreated
          if (result.updated) progress.updatedCount += 1
          else if (result.imported) {
            progress.importedCount += 1
            progress.readyCount += 1
          }
          ...
          progress.errorCount += 1
          const message = error instanceof Error ? error.message : 'Unknown ir-center vacancy processing error'
          await emitLog('ERROR', `ir-center vacancy ${row.externalId} failed: ${message}`)
          await emitProgress()
        }
      }
    } catch (error) {
      progress.errorCount += 1
      const message = error instanceof Error ? error.message : 'Unknown ir-center sync error'
  ...
  await emitLog(
    stoppedByLimit ? 'WARNING' : 'SUCCESS',
    stoppedByLimit ? 'Import stopped because the 350 MB limit was reached.' : 'ir-center import completed successfully.'
  )
  await emitProgress()
...
export async function syncHeadHunterVacancies(
  ...
  const providerRaw = (process.env.VACANCY_IMPORT_PROVIDER || '').trim().toLowerCase()
  const providers = providerRaw ? providerRaw.split('+').map((v) => v.trim()).filter(Boolean) : []

  if (providers.length > 0) {
    // run providers sequentially; each provider shares the same DB schema/output format
    let lastResult: SyncHeadHunterVacanciesResult | null = null
    for (const provider of providers) {
      if (provider === 'telegram') {
        lastResult = await syncTelegramVacancies(options)
        continue
      }
      if (provider === 'ircenter') {
        lastResult = await syncIrCenterVacancies(options)
        continue
      }
      if (provider === 'headhunter' || provider === 'hh') {
        // fallthrough to HH flow below
        lastResult = null
        break
      }
    }
